...
5. 建议包含保留高优先级、同源版本备选窗口、最近兼容站点、小范围错峰和人工处理。错峰保持卫星、频段和接触时长不变，在起始时间 ±15 分钟内按 1 分钟步长寻找同时满足本站容量、转向缓冲和同星不重叠的空位，取最小平移量（同量取更早）；找不到空位则不生成该建议，仅保留人工处理。评分显式记录优先级损失、站点距离、接触时长和资源余量；总分相同再按 action key 排序。
