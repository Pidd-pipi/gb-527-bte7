...
5. 建议包含保留高优先级、同源版本备选窗口、最近兼容站点、本站小范围错峰（`micro_time_shift`）和人工处理。评分显式记录优先级损失、站点距离、接触时长和资源余量；总分相同再按 action key 排序。
6. 小范围错峰保持卫星、频段和接触时长不变，只在原起始时间前后 15 分钟（含边界、按分钟递增，同距离优先提前）寻找空位；候选位置必须对同卫星窗口不重叠，并在本站原始区间和前后扩展 `slew_buffer_sec` 的区间上均不超过天线容量。锁定窗口从不参与错峰，15 分钟内没有空位时不生成该建议，冲突保留人工处理。建议卡片显示平移分钟数以及新旧 UTC 时段。
7. 锁定窗口从不进入自动移动列表。算法只生成建议，不调用外部设备，不写回 ContactWindow。
